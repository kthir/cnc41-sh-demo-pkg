...
def extract_subservice_vrf_plain_lsp_reachability_payload(root, test_run):
    ...
    #device_records = root.findall('.//devices/device/key')
    ...
    for device in devices:
        ...
        device_vrf_path = ".//devices/device[name='{device}']/config/vrf/vrf-list/name".format(device=device)
        device_vrf_records = root.findall(device_vrf_path)
        result["vrf"] = device_vrf_records[0].text

        # IP address on this PE side

        self_vpn_ip_path = ".//devices/device[name='{device}']/config/router/bgp/*/*/neighbor/id".format(device=device)
        ...
        all_vpn_ip_records_path = ".//devices/device/config/router/bgp/*/vrf/neighbor/id"
...
def extract_subservice_ebgp_nbr_health_payload(root, test_run):
    ...
    for device in get_pe_devices(root):
        ...
        ebgp_address_family_path = ".//devices/device[name='{device}']/config/router/bgp/*/*/neighbor/address-family/*".format(device=device)
        ...
        bgp_nbr_ipaddrs_path = ".//devices/device[name='{device}']/config/router/bgp/*/*/neighbor/id".format(device=device)
...
def extract_subservice_vpn_interfaces_payload(root, test_run):
    #print("Entering extract_subservice_vpn_interfaces_payload")
    result_list = []
    ...
    #pe_devices=get_pe_devices(root)
    #print("walking all devices")
    ...
    for device in get_pe_devices(root):
        ...
        result["device"] = device
        #print("device="+device)
        #if device not in pe_devices : continue
        #print("device is a PE")
        # Assumes exactly one PE-CE interface on this vrf/vpn
        interfacename_path=".//devices/device[name='{device}']/config/interface/*".format(device=device)
        interfaceid_path=".//devices/device[name='{device}']/config/interface/*/id".format(device=device)

        #interface_path = ".//vpn-nodes/vpn-node[ne-id='{device}']/vpn-network-accesses/vpn-network-access/port-id".format(
            #device=device)
        device_interfacename_record = root.xpath(interfacename_path)
        #print("Got " + str(len(device_interfacename_record)) + " Records")
        #for device_interfacename in device_interfacename_record:
            #print(device_interfacename.tag)     
            #print(device_interfacename)     

        device_interfaceid_record = root.xpath(interfaceid_path)
        #vlan_interface_path = ".//vpn-nodes/vpn-node[ne-id='{device}']/vpn-network-accesses/vpn-network-access/connection/tagged-interface/dot1q-vlan-tagged/cvlan-id".format(
            #device=device)
...
def get_pe_devices (root):
    #print("Getting list of PEs")
    xpathquery=".//pe-device"
    pe_device_records=root.xpath(xpathquery)
